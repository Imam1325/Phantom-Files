...
*   **🕵️ Полиморфные ловушки**: Модуль `Factory` генерирует уникальные файлы (изменяемые хеши, поддельные даты создания, динамический контент) используя `Faker` и `Jinja2`.
*   **👁️ Real-time Monitoring**: Сенсорная подсистема обнаруживает операции `OPEN`, `READ`, `COPY` за миллисекунды (используется абстракция над `inotify`).
...
├── deploy/             # Systemd unit-файлы и скрипты деплоя
├── resources/          # Шаблоны ловушек ("Золотые образы")
└── src/
    └── phantom/
        ├── core/       # Бизнес-логика и оркестрация событий
        ├── factory/    # Генерация полиморфных файлов
        ├── sensors/    # Драйверы мониторинга (Inotify/eBPF)
        └── sandbox/    # Управление Docker-окружением
```
